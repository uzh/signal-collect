...
class BeforeStorageDirectDeliveryAsynchronousWorker(
  mb: MessageBus[Any, Any],
  messageInboxFactory: QueueFactory,
  storageFactory: StorageFactory
  ) extends BeforeStorageAsynchronousWorker(mb, messageInboxFactory, storageFactory) {
  ...
  val pollingTimeoutNanoseconds: Long = idleTimeoutNanoseconds / 10
