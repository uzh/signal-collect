...
import com.esotericsoftware.kryo.serializers.FixedDeflateSerializer
...
class KryoSerializer(val system: ExtendedActorSystem) extends Serializer {
  ...
  private def getKryo(strategy: String, serializerType: String): Kryo = {
    ...
    try {
      ...
      def registerWithCompression(c: Class[_], id: Int) {
        kryo.register(c, id)
      ...
      case t: Throwable => 
        System.err.println(s"Unrecoverable error during Kryo initialization: ${t.getMessage}" )
        t.printStackTrace
    }
    kryo
  }
