...
/*
 * HashMap Int keys and Long values.
 */
class IntLongHashMap(
  ...
  private[this] final var values = new Array[Long](maxSize)
  ...
  final def foreach(f: (Int, Long) => Unit) {
    var i = 0
   ...
   */
  def put(key: Int, value: Long): Boolean = {
