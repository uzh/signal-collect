...
class IntIntHashMap(
  ...
  @inline final def foreach(f: (Int, Int) => Unit): Unit = {
    var i = 0
   ...
   */
  @inline final def process(f: (Int, Int) => Unit): Unit = {
    var i = 0
    var elementsProcessed = 0
    while (elementsProcessed < numberOfElements) {
      val key = keys(i)
  ...
  final def apply(key: Int): Int = get(key)
  ...
  @inline final def increment(key: Int): Unit = {
    var position = keyToPosition(key)
    var keyAtPosition = keys(position)
    while (keyAtPosition != 0 && key != keyAtPosition) {
      position = (position + 1) & mask
