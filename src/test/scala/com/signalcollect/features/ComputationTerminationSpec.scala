...
class ComputationTerminationSpec extends SpecificationWithJUnit with Mockito {
  ...
  "Global convergence" should {
    ...
    "work for asynchronous computations" in {
      ...
      if (aggregate > 4999.999999) {
        println("Computation converged completely instead of ending when the global constraint was met.")
      }
      ...
      println("Aggregation async global condition: (200.0 < x < 4999.99) " + aggregate)
      aggregate > 200.0 && aggregate < 4999.999999 && info.executionStatistics.terminationReason == TerminationReason.GlobalConstraintMet
    }
  }
